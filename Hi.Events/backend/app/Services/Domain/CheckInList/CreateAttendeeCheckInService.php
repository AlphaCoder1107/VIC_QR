...
class CreateAttendeeCheckInService
{
    ...
    ): CheckInResultDTO
    {
        ...
        $scanSlotLabels = json_decode($dbSettings->scan_slot_labels ?? '[]', true) ?: ['Entry'];

        $scanCount = \Illuminate\Support\Facades\DB::table('attendee_check_ins')
            ->where('attendee_id', $attendee->getId())
            ->whereNull('deleted_at')
            ->count();

        if ($scanCount >= $maxScans) {
            $existingCheckIn = $this->getExistingCheckIn($existingCheckIns, $attendee);
            return new CheckInResultDTO(
                checkIn: $existingCheckIn,
                error: __('Attendee :attendee_name has exhausted all scans', [
        ...
        return $this->db->transaction(function () use ($attendee, $checkInList, $checkInAction, $checkInUserIpAddress, $scanCount, $scanSlotLabels, $maxScans) {
            $checkIn = $this->createCheckIn($attendee, $checkInList, $checkInUserIpAddress, $scanCount, $scanSlotLabels, $maxScans);
    ...
    private function createCheckIn(
        ...
        string                  $checkInUserIpAddress,
        int                     $scanCount,
        array                   $scanSlotLabels,
        int                     $maxScans
    ): AttendeeCheckInDomainObject
    {
        $scanSlot = $scanCount + 1;
        $slotLabel = $scanSlotLabels[$scanCount] ?? ('Scan ' . $scanSlot);

        $checkIn = $this->attendeeCheckInRepository->create([
        ...
        if (method_exists($checkIn, 'setScanSlot')) {
            $checkIn->setScanSlot($scanSlot);
        }
        if (method_exists($checkIn, 'setSlotLabel')) {
            $checkIn->setSlotLabel($slotLabel);
        }
        if (method_exists($checkIn, 'setMaxScans')) {
            $checkIn->setMaxScans($maxScans);
        }
