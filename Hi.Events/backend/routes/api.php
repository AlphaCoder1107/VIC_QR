...
use HiEvents\Http\Actions\Webhooks\RazorpayWebhookAction;
...
use HiEvents\Http\Actions\Enrollment\Public\GetEnrollmentLookupActionPublic;
use HiEvents\Http\Actions\Enrollment\Public\InitiateRazorpayOrderAction;
use HiEvents\Http\Actions\Enrollment\Public\ClaimFreeTicketAction;
use HiEvents\Http\Actions\Enrollment\Public\UpdateEnrollmentEmailAction;
use HiEvents\Http\Actions\Enrollment\Public\ResendEnrollmentTicketAction;
use HiEvents\Http\Actions\Enrollment\Public\VerifyRazorpayPaymentAction;
...
$router->middleware(['auth:api'])->group(
    function (Router $router): void {
        ...
        // Student Roster (VIC Custom)
        $router->post('/auth/organiser/{organiser}/events/{event}/roster/import', \HiEvents\Http\Actions\Organisers\Events\ImportStudentRosterAction::class);
        $router->get('/auth/organiser/{organiser}/events/{event}/roster', \HiEvents\Http\Actions\Organisers\Events\GetStudentRosterAction::class);
        $router->get('/auth/organiser/{organiser}/events/{event}/roster/template', \HiEvents\Http\Actions\Organisers\Events\DownloadRosterTemplateAction::class);
        $router->delete('/auth/organiser/{organiser}/events/{event}/roster/{roster}', \HiEvents\Http\Actions\Organisers\Events\DeleteStudentRosterAction::class);

        // Payment Verifications (VIC Custom)
        $router->get('/auth/organiser/{organiser}/events/{event}/payment-verifications', \HiEvents\Http\Actions\Organisers\Events\PaymentVerifications\ListPaymentVerificationsAction::class);
        $router->post('/auth/organiser/{organiser}/events/{event}/payment-verifications/import', \HiEvents\Http\Actions\Organisers\Events\PaymentVerifications\ImportPaymentVerificationsAction::class);
        $router->post('/auth/organiser/{organiser}/events/{event}/payment-verifications/{id}/verify', \HiEvents\Http\Actions\Organisers\Events\PaymentVerifications\VerifyPaymentSubmissionAction::class);
        $router->post('/auth/organiser/{organiser}/events/{event}/payment-verifications/{id}/reject', \HiEvents\Http\Actions\Organisers\Events\PaymentVerifications\RejectPaymentSubmissionAction::class);

        // Free Ticket Prefixes & Price (VIC Custom)
        $router->get('/auth/organiser/{organiser}/events/{event}/free-prefixes', \HiEvents\Http\Actions\Organisers\Events\ListFreeTicketPrefixesAction::class);
        $router->post('/auth/organiser/{organiser}/events/{event}/free-prefixes', \HiEvents\Http\Actions\Organisers\Events\CreateFreeTicketPrefixAction::class);
        $router->delete('/auth/organiser/{organiser}/events/{event}/free-prefixes/{prefix}', \HiEvents\Http\Actions\Organisers\Events\DeleteFreeTicketPrefixAction::class);
        $router->patch('/auth/organiser/{organiser}/events/{event}/ticket-price', \HiEvents\Http\Actions\Organisers\Events\UpdateTicketPriceAction::class);

        // Pricing Rules (VIC Custom)
        $router->get('/events/{event}/pricing-rules', \HiEvents\Http\Actions\Organisers\Events\Pricing\ListPricingRulesAction::class);
        $router->post('/events/{event}/pricing-rules', \HiEvents\Http\Actions\Organisers\Events\Pricing\CreatePricingRuleAction::class);
        $router->put('/events/{event}/pricing-rules/{rule}', \HiEvents\Http\Actions\Organisers\Events\Pricing\UpdatePricingRuleAction::class);
        $router->delete('/events/{event}/pricing-rules/{rule}', \HiEvents\Http\Actions\Organisers\Events\Pricing\DeletePricingRuleAction::class);
    }
);
...
$router->prefix('/public')->group(
    function (Router $router): void {
        ...
        // Enrollment lookup (VIC Custom)
        $router->middleware(['throttle:10,1'])->group(function (Router $router): void {
            $router->post('/events/{event_id}/enrollment-lookup', GetEnrollmentLookupActionPublic::class);

            // Update enrollment email address
            $router->patch('/events/{event_id}/enrollment/update-email', UpdateEnrollmentEmailAction::class);

            // Resend enrollment ticket email
            $router->post('/events/{event_id}/enrollment/resend-ticket', ResendEnrollmentTicketAction::class);
        });

        $router->middleware(['throttle:5,1'])->group(function (Router $router): void {
            // Razorpay enrollment order initiation
            $router->post('/events/{event_id}/enrollment/initiate-order', InitiateRazorpayOrderAction::class);

            // Claim free ticket for eligible enrollments
            $router->post('/events/{event_id}/enrollment/claim-free', ClaimFreeTicketAction::class);

            // Verify Razorpay payment and complete order
            $router->post('/events/{event_id}/enrollment/verify-payment', VerifyRazorpayPaymentAction::class);
        });
        ...
        $router->post('/webhooks/razorpay', RazorpayWebhookAction::class);
