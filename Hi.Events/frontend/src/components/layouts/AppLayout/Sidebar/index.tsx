    ...
    return (
        <div className={classNames(`${classes.sidebar} ${sidebarOpen ? classes.open : classes.closed}`)}>
            <div className={classes.logo}>
                <NavLink to={`/manage/events`} style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
                    {getConfig("VITE_APP_LOGO_LIGHT") ? (
                        <img
                            style={{maxWidth: '160px', margin: "10px auto"}}
                            src={getConfig("VITE_APP_LOGO_LIGHT")}
                            alt={t`${getConfig("VITE_APP_NAME", "CodePode")} logo`}
                        />
                    ) : (
                        <span style={{ fontSize: '1.4rem', fontWeight: 800, color: '#fff', letterSpacing: '0.05em', fontFamily: 'Outfit, Inter, sans-serif' }}>
