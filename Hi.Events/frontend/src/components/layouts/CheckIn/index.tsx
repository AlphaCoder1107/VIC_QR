...
const CheckIn = () => {
    ...
    const [flash, setFlash] = useState<{
        type: 'success' | 'exhausted' | 'invalid';
        message: string;
    } | null>(null);

    const showFlash = useCallback((type: 'success' | 'exhausted' | 'invalid', message: string) => {
    ...
    const handleCheckInAction = (attendee: Attendee, action: 'check-in' | 'check-in-and-mark-order-as-paid') => {
        checkInMutation.mutate({
            ...
            onSuccess: (res) => {
                ...
                const checkInInfo = data?.find((c: any) => c.attendee_id === attendee.id);
                const n = checkInInfo?.scan_slot ?? 1;
                const max = checkInInfo?.max_scans ?? 1;
                const label = checkInInfo?.slot_label ?? 'Entry';

                showFlash('success', `Scan ${n} of ${max} · ${label}`);
            ...
            onError: (error) => {
                ...
                if (error instanceof AxiosError) {
                    if (error.response?.status === 409 && error.response?.data?.status === 'exhausted') {
                        const {scanned, max} = error.response.data;
                        showFlash('exhausted', `All ${max} scans used for this ticket`);
                    } else {
    ...
    const handleQrCheckIn = useCallback(async (attendeePublicId: string) => {
        ...
        if (!attendee) {
            try {
                ...
                showFlash('invalid', t`Invalid or unknown QR code`);
        ...
        isProcessingRef.current = false;
    }, [attendees, checkInListShortId, allowOrdersAwaitingOfflinePaymentToCheckIn, checkInModalHandlers, handleCheckInAction, playErrorSound, showFlash]);
        ...
        <div className={classes.container}>
            ...
            {flash && (
                <div style={{
                    ...
                    width: '100vw',
                    height: '100vh',
                    backgroundColor: flash.type === 'success' ? '#2b8a3e' : '#c92a2a',
                    background: flash.type === 'success' 
                        ? 'linear-gradient(135deg, #2b8a3e 0%, #0b7285 100%)' 
                        : 'linear-gradient(135deg, #c92a2a 0%, #862e2e 100%)',
                    color: '#ffffff',
                    display: 'flex',
                    ...
                    <div style={{
                        ...
                        {flash.type === 'success' ? '✅' : flash.type === 'exhausted' ? '❌' : '⛔'}
                    </div>
                    <div style={{
                        fontSize: '3rem',
                        fontWeight: 800,
